...
core.Document.prototype._elementBuildersNS[SVG_NS] = {};
	...
	tagNames.forEach(function(tag) {
		...
		ns.forEach(function(n) {
			if(!n) {
				...
				core.Document.prototype._elementBuildersNS[n][tag.toLowerCase()] = creation;
			}
 ...
 * 	current subpath. If a "closepath" is followed immediately by a "moveto", 
 * 	then the "moveto" identifies the start point of the next subpath. If a 
 * 	"closepath" is followed immediately by any other command, then the next 
 * 	subpath starts at the same initial point as the current subpath.
 * l/h/v = lineto(x, y)
 * 	l = relative, L = absolute
 * 	h(x) = horizontal relative, H = horizontal absolute
 * 	v(y) = vertical relative, V = vertical absolute
 * c/s = Bézier
...
define("SVGPathElement", {
	...
	proto: {
		getBBox: function() {
			//Okay, this is going to be a major pain.
			return (new Path(this.d).getBBox());
		}
	},
	parentClass: core.SVGShapeElement,
	attributes: [
...
define("SVGPolylineElement", {
	...
	proto: {
		getBBox: function() {
			var path = this.points.replace(/([\d.-]+)[,\s]+([\d.-]+)/g, "M$1,$2");
			return (new Path(path).getBBox());
		}
	},
	attributes: [
	'points'
